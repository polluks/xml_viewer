    ...
    if (len > 0)
    {
        if ((s[0] != '\n') && (s[0] != 0x20)  && (s[0] != '\t'))
        {
            if ((tmp = (struct xml_data*)AllocVec(sizeof(struct xml_data), MEMF_ANY)))
            {
                ...
                {
                     // initialize the list even though it will remain empty
                     #ifdef __MORPHOS__
                     ...
                     if ((buffer=(char *)AllocVec(len+3, MEMF_ANY)))
                     {
                         ...
                         //KPrintF("------TXT_%s_%d\n", buffer, len);
                         tmp->type = XML_TEXT;
                         my_tree->tn[my_tree->depth+1] = (struct MUIS_Listtree_TreeNode *)DoMethod(my_tree->tree, MUIM_Listtree_Insert, buffer,
                                                         (APTR)tmp, my_tree->tn[my_tree->depth], MUIV_Listtree_Insert_PrevNode_Tail, 0);
                         FreeVec(buffer);
                     }
                     else  // do not insert but still clean up the list and node
                     {
    ...
    if ((tmp = (struct xml_data*)AllocVec(sizeof(struct xml_data), MEMF_ANY)))
    {
        ...
        if ((tmp->attr_list =  (struct  MinList*)AllocVec( sizeof( struct MinList), MEMF_ANY)))
        {
            ...
            while(atts[i])
            {
                ...
                {
                    // allocate memory for attributes and add them to the list
                    if((dn->atributes = (char *)AllocVec(strlen(atts[i])+1, MEMF_ANY)))
                        strcpy(dn->atributes, atts[i]);

                    // allocate memory for values and add them to the list
            ...
            tmp->type = XML_VALUES;
            my_tree->tn[my_tree->depth+1] = (struct MUIS_Listtree_TreeNode *)DoMethod(my_tree->tree, MUIM_Listtree_Insert, name,
...
///
/// decl_hndl()
// process xml declaration header
                ...
                const XML_Char *encoding, int standalone)
{
    ...
    if ((tmp = (struct xml_data*)AllocVec(sizeof(struct xml_data), MEMF_ANY)))
    {
        ...
        if ((tmp->attr_list = (struct  MinList*)AllocVec( sizeof(struct MinList), MEMF_ANY)))
        {
            ...
            my_tree->tn[0] =  (struct MUIS_Listtree_TreeNode *)DoMethod(my_tree->tree, MUIM_Listtree_Insert, my_tree->filename, (LONG)tmp,  MUIV_Listtree_Insert_ListNode_Root, MUIV_Listtree_Insert_PrevNode_Tail, TNF_LIST);
        }
        else
            FreeVec(tmp);
    }
